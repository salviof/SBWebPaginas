...
import com.super_bits.modulosSB.webPaginas.push.FabTipoMensagemPush;
    ...
    public boolean publicar(ComoDialogo pDialogo) {
        ...
        if (pDialogo.getPaginaInstanciaID() != null) {
            return pushContext.enviar(
                    pDialogo.getDestinatario().getUsuario().getEmail(), FabTipoMensagemPush.EXIBIR_DIALOGO_TRANSITORIO.gerarPayloadPush(pDialogo));
        } else {
            return pushContext.enviar(
                    pDialogo.getDestinatario().getUsuario().getEmail(), FabTipoMensagemPush.ATUALIZAR_NOTIFICACOES.gerarPayloadPush(pDialogo));
        }
